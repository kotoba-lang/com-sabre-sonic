(ns sabre-sonic.main-test
  "Contract + behavioral test for the sabre-sonic L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "sabre_sonic-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'sabre-sonic.main-test)]
